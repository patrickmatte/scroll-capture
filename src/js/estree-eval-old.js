export function evaluate(node, scope, tabs = '') {
  delete node.start;
  delete node.end;
  ...
  tabs += '    ';
  return evaluateFns[node.type](node, scope, tabs);
}

function evaluateSpreadArray(elements, scope) {
  console.log('????? evaluateSpreadArray');
  const result = [];
...
const evaluateFns = {
  ...
  Identifier(node, scope, tabs) {
    console.log(tabs, 'Identifier node=', node, 'scope=', scope);
    return scope[node.name];
  },

  Literal(node, scope, tabs) {
    console.log(tabs, 'Literal node=', node, 'scope=', scope);
    return node.value;
  },
  ...
  MemberExpression(node, scope, tabs) {
    console.log(tabs, 'MemberExpression', node, scope);

    switch (node.object.type) {
      case 'Super':
        throw new Error(`Cannot call ${node.object.type}`);
      default:
        const object = evaluate(node.object, scope, tabs);
        if (node.computed) {
          return object[evaluate(node.property, scope, tabs)];
        } else {
          return evaluate(node.property, object, tabs);
        }
    }
  },
